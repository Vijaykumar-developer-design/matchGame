...
class App extends Component {
  ...
  generateRandomImg = id => {
    const {initialImagesList, bigImage} = this.state

    const randomImgNum = Math.floor(Math.random() * initialImagesList.length)
    const randomItem = imagesList[randomImgNum]
  ...
  getTabId = id => {
    this.setState({thumbnails: id})
  }

  renderGameStarted = () => {
    const {bigImage, initialImagesList, thumbnails} = this.state

    const filteredData = initialImagesList.filter(
      eachImage => eachImage.category === thumbnails,
    )

    return (
      <div>
        <div className="background-down">
          <img className="image" alt="match" src={bigImage} />
        </div>
          ...
          <ul className="images-list">
            {filteredData.map(eachImage => (
              <ThumbNail
                Details={eachImage}
                key={eachImage.id}
                generateRandomImg={this.generateRandomImg}
              />
            ))}
          </ul>
        </div>
  ...
  renderGameEnd = () => {
    const {score, initialTime, gameStarted} = this.state
    if (initialTime === 1 || gameStarted === false) {
      return (
        ...
        <nav className="nav-bar">
          ...
          <ul className="score-card">
            ...
            <div className="time-score-container">
              ...
              />
              <p className="time-count">{initialTime} sec</p>
            </div>
          </ul>
        </nav>
        {gameStarted ? this.renderGameStarted() : this.renderGameEnd()}
      </div>
    )
  }
}
